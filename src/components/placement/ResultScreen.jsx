...
export default function ResultScreen({ score, total, level, onRestart }) {
  ...
  useEffect(() => {
    ...
    el.offsetHeight;
    el.style.transition = 'opacity 0.45s ease, transform 0.5s cubic-bezier(.34,1.56,.64,1)';
    el.style.opacity = '1';
  ...
  return (
    <div ref={wrapRef} style={{ textAlign:'center', opacity:0 }}>
      ...
      <p style={{
        color:'#c4b5fd', fontWeight:800, fontSize:11,
        letterSpacing:'2.5px', textTransform:'uppercase', marginBottom:6,
        animation: 'rsLabelIn 0.4s 0.3s ease both',
      }}>
        Test Complete
      </p>
      <h2 style={{
        ...
        animation: 'rsLabelIn 0.4s 0.4s ease both',
      }}>
        Your English Level
      </h2>
      ...
      <div
        ...
        style={{
          ...
          marginBottom:26, minWidth:270,
          opacity:0,
        }}
      >
        <div style={{ fontSize:46, marginBottom:10, animation:'rsBounce 0.5s 0.6s cubic-bezier(.34,1.56,.64,1) both' }}>
          {cfg.emoji}
        </div>
        <div style={{
          fontSize:60, fontWeight:900,
          color:cfg.color, textShadow:`0 0 32px ${cfg.color}`,
          lineHeight:1, letterSpacing:'-3px', marginBottom:5,
          animation:'rsBounce 0.5s 0.7s cubic-bezier(.34,1.56,.64,1) both',
        }}>
          {level}
        </div>
        <div style={{ fontSize:19, fontWeight:800, color:'#e2e8f0', marginBottom:8, animation:'rsLabelIn 0.4s 0.8s ease both' }}>
          {cfg.name}
        </div>
        <div style={{ fontSize:13, color:'#9ca3af', maxWidth:250, lineHeight:1.58, animation:'rsLabelIn 0.4s 0.9s ease both' }}>
          {cfg.desc}
        </div>
        ...
        <div style={{
          ...
          animation:'rsLabelIn 0.4s 1s ease both',
        }}>
      ...
      <div style={{
        ...
        animation:'rsLabelIn 0.4s 1.1s ease both',
      }}>
        <button
          onClick={onRestart}
          style={{
            ...
            borderRadius:15, padding:'12px 24px',
            color:'#c4b5fd', fontSize:13, fontWeight:800,
            cursor:'pointer', fontFamily:'inherit',
            transition:'all 0.2s',
          }}
          onMouseEnter={e => { e.currentTarget.style.background='rgba(139,92,246,0.26)'; e.currentTarget.style.transform='translateY(-1px)'; }}
